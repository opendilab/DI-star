...
import numpy as np
import torch
from ctools.torch_utils import get_tensor_data
...
class HiddenStateHelper(IAgentStatefulPlugin):
    ...
    def register(
        ...
        agent.hidden_state = state_manager._state
        ...
        def forward_state_wrapper(forward_fn):
            ...
            def wrapper(data, **kwargs):
                ...
                if h:
                    agent._state_manager.after_forward(h, state_info, valid_id)
                if save_prev_state:
    ...
    def after_forward(self, h: Any, state_info: dict, valid_id: list) -> None:
        assert len(h) == len(state_info), '{}/{}'.format(len(h), len(state_info))
        for i, idx in enumerate(state_info.keys()):
            if valid_id is None or idx in valid_id:
                self._state[idx] = h[i]
