...
from ctools.pysc2.env import environment
from ctools.pysc2.lib import actions as actions_lib
from ctools.pysc2.lib import features
from ctools.pysc2.lib import metrics
from ctools.pysc2.lib import portspicker
from ctools.pysc2.lib import renderer_human
...
from ctools.pysc2.env.sc2_env import Agent, Bot
...
class SC2EVALEnv(environment.Base):
    """A Starcraft II environment.
    ...
    def __init__(self,  # pylint: disable=invalid-name
        ...
        if game_type == 'human_vs_agent':
            self.active_agent_num = 1
        elif game_type == 'agent_vs_agent':
            self.active_agent_num = 2
    ...
    def _launch_game(self):
        ...
        if self.game_type == 'human_vs_agent':
            self._sc2_procs = []
            self._sc2_procs.append(self._run_config.start(extra_ports=self._ports,
                                                          want_rgb=False,
                                                          ))
            self._sc2_procs.append(self._run_config.start(extra_ports=self._ports,
                                                          want_rgb=False,
    ...
    def _create_join(self, require_features=True):
    ...
    def step(self, actions, step_mul=None):
        ...
        for c, f, o, a in zip(self._controllers, self._features, self._obs, actions):
            if not type(a) == list:  # a single command, must be ctools.pysc2 func-call action
        ...
        return self._step(step_mul), results
    ...
    def _get_observations(self, target_game_loop):
        ...
        with self._metrics.measure_observation_time():
            self._obs[:self.active_agent_num], self._agent_obs[:self.active_agent_num] = zip(*self._parallel.run(
                (parallel_observe, c, f)
                for c, f in zip(self._controllers[:self.active_agent_num], self._features[:self.active_agent_num])))
        # with self._metrics.measure_observation_time():
        ...
        if (game_loop < target_game_loop and
                not any(o.player_result for o in self._obs[:self.active_agent_num])):
            raise ValueError(
        ...
        if self._realtime:
            ...
            if self._last_obs_game_loop is not None:
                for i, obs in enumerate(self._obs[:self.active_agent_num]):
    ...
    def _observe(self, target_game_loop):
        ...
        episode_complete = any(o.player_result for o in self._obs[:1])

        if episode_complete:
            self._state = environment.StepType.LAST
            discount = 0
            for i, o in enumerate(self._obs[:self.active_agent_num]):
        ...
        if self._state == environment.StepType.LAST:
            ...
            logging.info(("Episode %s finished after %s game steps. "
                         ...
                         [o["score_cumulative"][0] for o in self._agent_obs[:self.active_agent_num]])
