...
class FiLMedResBlock(nn.Module):
  ...
  def forward(self, x, gammas: Optional[torch.Tensor] = None, betas: Optional[torch.Tensor] = None, extra_channels: Optional[torch.Tensor] = None, cond_maps: Optional[torch.Tensor] = None):
...
class LSTM(nn.Module):
    ...
    def forward(self, inputs, prev_state: Tuple[Tensor, Tensor], list_next_state: bool = False, forget_bias: float = 1.0):
        '''
        Input:
