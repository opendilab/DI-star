...
from ..module_utils import Transformer
...
class BeginningBuildOrderEncoder(nn.Module):
    def __init__(self, cfg, bo_cfg):
        ...
        self.transformer = Transformer(
            input_dim=self.input_dim, head_dim=self.cfg.head_dim, hidden_dim=self.cfg.output_dim * 2, output_dim=self.cfg.output_dim)
        self.embedd_fc = fc_block(self.cfg.output_dim, self.output_dim, activation=self.act)
        self.action_one_hot = nn.Embedding.from_pretrained(torch.eye(self.cfg.action_one_hot_dim),freeze=True,padding_idx=None)
        self.order_one_hot = nn.Embedding.from_pretrained(torch.eye(20),freeze=True,padding_idx=None)
        self.location_binary = nn.Embedding.from_pretrained(get_binary_embed_mat(self.cfg.binary_dim),freeze=True,padding_idx=None)
...
class ScalarEncoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.position_array = torch.nn.Parameter(compute_denominator(torch.arange(0, self.cfg.module.time.output_dim, dtype=torch.float), self.cfg.module.time.output_dim), requires_grad=False)
