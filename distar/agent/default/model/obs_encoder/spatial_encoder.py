...
from distar.ctools.torch_utils import conv2d_block, fc_block, build_activation, ResBlock, same_shape
...
class SpatialEncoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.project = conv2d_block(self.cfg.input_dim, self.cfg.project_dim, 1, 1, 0, activation=self.act, norm_type=self.norm)
        ...
        if self.head_type == 'fc':
            self.fc = fc_block(dim * self.whole_cfg.model.spatial_y // 8 * self.whole_cfg.model.spatial_x // 8, self.cfg.fc_dim, activation=self.act)
        else:
