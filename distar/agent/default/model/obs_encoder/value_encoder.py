from distar.pysc2.lib.static_data import BEGIN_ACTIONS
import torch
import torch.nn as nn
import torch.nn.functional as F
from distar.ctools.torch_utils import fc_block, build_activation, conv2d_block, ResBlock, same_shape
from .spatial_encoder import SpatialEncoder
from ..module_utils import Transformer, scatter_connection
...
class ValueEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        for k, item in self.cfg.modules.items():
            if item['arc'] == 'fc':
                ...
                self.encode_modules[k] = nn.Embedding(num_embeddings=item['num_embeddings'], embedding_dim=item['embedding_dim'])
        ...
        self.scatter_project = fc_block(self.cfg.scatter.scatter_input_dim, self.cfg.scatter.scatter_dim, activation=self.act)
        self.scatter_type = self.cfg.scatter.scatter_type
        self.scatter_dim = self.cfg.scatter.scatter_dim

        self.project = conv2d_block(self.cfg.spatial.input_dim, self.cfg.spatial.project_dim, 1, 1, 0, activation=self.act)
        down_layers = []
        ...
        for i in range(len(self.cfg.spatial.down_channels)):
            ...
            down_layers.append(
                conv2d_block(dims[i], dims[i + 1], 3, 1, 1, activation=self.act)
            )
        ...
        self.spatial_fc = fc_block(dim * self.whole_cfg.model.spatial_y // 8 * self.whole_cfg.model.spatial_x // 8, self.cfg.spatial.spatial_fc_dim, activation=self.act)
    ...
    def forward(self, x):
        ...
        scatter_map = scatter_connection((b, h, w), project_embedding, entity_location, self.scatter_dim, self.scatter_type)
