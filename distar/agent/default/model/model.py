...
class Model(nn.Module):
    ...
    def compute_logp_action(self,
        ...
        return {'action_info': action_info, 'action_logp': log_action_probs, 'selected_units_num': selected_units_num,
                'entity_num': entity_num,
                'hidden_state': out_state, 'logit': logit, 'extra_units': extra_units}
    ...
    def compute_teacher_logit(self,
                              ...
                              selected_units_num,
                              action_info: Dict[str, Tensor],
                              **kwargs):
        lstm_input, scalar_context, baseline_feature, entity_embeddings, map_skip = \
            self.encoder(spatial_info, entity_info, scalar_info, entity_num)
        lstm_output, out_state = self.core_lstm(lstm_input.unsqueeze(dim=0), hidden_state)
        action_info, selected_units_num, logit = self.policy.train_forward(lstm_output.squeeze(dim=0),
                                                                           entity_embeddings, map_skip,
                                                                           scalar_context, entity_num, action_info,
                                                                           selected_units_num)
        return {'logit': logit, 'hidden_state': out_state, 'entity_num': entity_num,
                'selected_units_num': selected_units_num}

    def rl_learner_forward(self,
                           spatial_info,
                           entity_info,
                           scalar_info,
                           entity_num,
        ...
        hidden_state = [[hidden_state[i][j].view(-1, batch_size, hidden_size)[0,:,:] for j in range(len(hidden_state[i]))] for i in range(len(hidden_state))]
        ...
        _, _, logits = self.policy.train_forward(policy_lstm_input, policy_entity_embeddings,
                                                 policy_map_skip, policy_scalar_context, policy_entity_num,
        ...
        logits['selected_units'] =  torch.nn.functional.pad(
            logits['selected_units'], ( 0,0, 0, MAX_SELECTED_UNITS_NUM - logits['selected_units'].shape[2],), 'constant',
            -1e9)
