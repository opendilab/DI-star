...
from ctools.pysc2.lib.features import FeatureUnit
from ctools.pysc2.lib.action_dict import ACT_TO_GENERAL_ACT, ACT_TO_GENERAL_ACT_ARRAY
from ctools.pysc2.lib.static_data import NUM_BUFFS, NUM_ABILITIES, NUM_UNIT_TYPES, UNIT_TYPES_REORDER,\
...
from ctools.envs.common import EnvElement, num_first_one_hot, sqrt_one_hot, div_one_hot,\
        ...
        for item in self.template:
            ...
            if key in idx_dict.keys():
                ...
                if key == 'height_map':
                    ...
                    data = torch.nn.functional.one_hot(data, item['dim'])
                    data = data.permute(2, 0, 1)
                ret.append(data.float())
        return ret
...
class EntityObs(EnvElement):
    ...
    def parse(self, obs: dict) -> torch.Tensor:
        ...
        y = torch.from_numpy(feature_unit[:, 13]).unsqueeze(dim=1)
        x = torch.from_numpy(feature_unit[:, 12]).unsqueeze(dim=1)
        # entity_raw['location'].append([feature_unit[idx].y, feature_unit[idx].x])
        # entity_raw['id'] = feature_unit[idx].tag
        # entity_raw['type'].append(int(feature_unit[idx].unit_type))
        entity_raw['id'] = feature_unit[:, 29]
        entity_raw['type'] = feature_unit[:, 0]
        entity_raw['location'] = torch.cat([y, x], dim=1)
        ...
        for idx, item in enumerate(self.template):
            ...
            item_data = torch.from_numpy(item_data).long()
...
class ScalarObs(EnvElement):
    ...
    def _init(self, cfg: dict) -> None:
        ...
        self.template = [
            ...
            },
            {
                ...
                'op': tensor_wrapper(partial(one_hot, num=5)),
                'value': {
                    'min': 0,
                    'max': 1,
                    'dtype': float,
            ...
            {
                'key': 'immediate_beginning_build_order',
                'dim': NUM_BEGIN_ACTIONS + LOCATION_BIT_NUM * 2 + self.begin_num,
                'ori': 'immediate_beginning_build_order',
                'op': lambda x: x,
                'value': {
                    'min': 0,
                    'max': 1,
            ...
            self.template += [
                {
                    ...
                    'op': lambda x: torch.log(1 + torch.FloatTensor(x).clamp_(min=0)),
                    'value': {
                        'min': 0,
                        'max': 'inf',
                        'dtype': float,
    ...
    def _parse_last_queued_repeat(self, last_action: dict) -> torch.Tensor:
        ...
        last_queued = torch.nn.functional.one_hot(last_queued, 3).squeeze(dim=0).float()
        last_repeat = torch.zeros(17)
