...
def get_enemy_upgrades_raw_data(units, upgrades=None):
    """
        Overview: get enemy upgrades according to the current obs and the last upgrades
        Arguments:
            - units (:obj:`np.ndarray`): the raw units of ctools.pysc2 observation
    ...
    #         #         if level > 1:
    #         #             upgrades[22 + level] = 0
    #         if unit.unit_type in zerg_melee_unit_types:
    #             if unit.attack_upgrade_level > 0:
    #                 level = unit.attack_upgrade_level
    #                 upgrades[32 + level] = 1
    #                 if level > 1:
    ...
    #                 if level > 1:
    #                     upgrades[43 + level] = 0
    # old_upgrades = upgrades.clone()
    # print('old upgrad', time.time() - t)

    if upgrades is None:
        upgrades = torch.zeros(48).long()
    enemy_units_idx = np.where([units[:, 'alliance'] == 4])[0]
    enemy_units = units[enemy_units_idx]

    for types, idx in zip([zerg_melee_unit_types, zerg_missile_unit_types, zerg_flyer_unit_types], [32, 38, 41]):
        units_set = set()
        for t in types:
            idx = np.where(enemy_units == t)[0]
            units_set.union(idx)
        attack_upgrad_level = enemy_units[list(units_set)][:, 'attack_upgrade_level']
        if len(attack_upgrad_level):
            attack_upgrad_level = max(attack_upgrad_level)
            if attack_upgrad_level > 0:
                upgrades[idx + attack_upgrad_level] = 1
                if attack_upgrad_level > 1:
                    upgrades[idx - 1 + attack_upgrad_level] = 0



        armor_upgrade_level = enemy_units[list(units_set)][:, 'armor_upgrade_level']
        if len(armor_upgrade_level):
            armor_upgrade_level = max(armor_upgrade_level)
            if armor_upgrade_level > 0:
                upgrades[idx + 3 + armor_upgrade_level] = 1
                if armor_upgrade_level > 1:
                    upgrades[idx + 2 + armor_upgrade_level] = 0
...
def get_enemy_upgrades_processed_data(obs, upgrades):
    """
        Overview: get enemy upgrades according to the current obs and the last upgrades
        Arguments:
            - obs (:obj:`dict`): the dict of current obs(ctools.pysc2 obs)
            - upgrades (:obj:`torch.Tensor or None`): for the first frame, upgrades is None, otherwise
                upgrades is a shape([48]) tensor that is the last upgrades
        Returns:
            - upgrades (:obj:`torch.Tensor`): the current upgrades
