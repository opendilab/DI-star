...
class AlphaStarObsRunner(EnvElementRunner):
    ...
    def _init(self, cfg: dict) -> None:
        ...
        self._entity_clip_num = cfg.get('entity_clip_num', 512)
    ...
    def _mask_obs(self, obs: dict) -> dict:
        obs['entity_info'] = obs['entity_info'][:self._entity_clip_num]
        for k, v in obs['entity_raw'].items():
            obs['entity_raw'][k] = v[:self._entity_clip_num]
        if self._ignore_camera:
            obs['spatial_info'][1:3] *= 0
            obs['entity_info'][:, 408:410] *= 0
            obs['scalar_info']['last_queued'][3:] *= 0
        return obs
    ...
    def _merge_stat2obs(self, obs, engine, agent_no, game_loop=None):
        ...
        obs.update(stat)
        immediate_stat = engine.episode_stat[agent_no].get_input_z()
        obs['immediate_beginning_build_order'] = immediate_stat['beginning_build_order']
        obs['immediate_cumulative_stat'] = immediate_stat['cumulative_stat']
        return obs
