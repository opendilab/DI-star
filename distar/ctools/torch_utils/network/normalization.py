...
from distar.ctools.utils import get_group, try_import_link

link = try_import_link()


class GroupSyncBatchNorm(torch.nn.SyncBatchNorm):
    """
    Overview:
    ...
    def __init__(
        self, num_features, bn_group_size=None, momentum=0.1, sync_stats=True, var_mode=None
    ):
        """
        Overview:
        ...
        super(GroupSyncBatchNorm, self).__init__(
            num_features,
            momentum=momentum,
            process_group=get_group(bn_group_size)
        )
