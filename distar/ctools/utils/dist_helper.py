...
# import torch
#
# from distar.ctools.utils import FakeLink
# from distar.ctools.utils.import_helper import try_import_link
# from distar.ctools.utils.default_helper import error_wrapper
#
...
#
#
# def allreduce(data, op='sum', reduce=True):
#     r"""
#     Overview:
...
def allreduce(data, op='sum', reduce=True):
    ...
    Arguments:
        ...
        - op (:obj:`str`): the operation to perform on data, support ['sum', 'max']
    """
    link_op_map = {'sum': link.ReduceOp.SUM, 'max': link.ReduceOp.MAX, 'min': link.ReduceOp.MIN}
    if op not in link_op_map.keys():
        raise KeyError("not support allreduce op type: {}".format(op))
    else:
        link_op = link_op_map[op]
    if is_fake_link:
        return data
    link.all_reduce(data, op=link_op)
    if op == 'sum' and reduce:
        data.div_(get_world_size())
...
def dist_init(method='slurm', init_method='env://', rank=0, world_size=0, device_id=0):
    r"""
    Overview:
        init the distribution
    Arguments:
    ...
    link.init_process_group(backend='nccl', init_method=init_method, rank=rank, world_size=world_size)
    world_size = link.get_world_size()
    rank = link.get_rank()
...
def simple_group_split(world_size, rank, num_groups):
    ...
    rank_list = [list(map(int, x)) for x in rank_list]
    for i in range(num_groups):
        groups.append(link.new_group(rank=rank_list[i], backend='nccl'))
    group_size = world_size // num_groups
    return groups[rank // group_size]
...
class DistModule(torch.nn.Module):
    ...
    def _make_hook(self, name, p, i):
        ...
        def hook(*ignore):
            link.all_reduce(name, p.grad.data)
