...
from distar.ctools.utils import allreduce
...
class LoadCkptHook(LearnerHook):
    ...
    def __call__(self, engine: 'BaseLearner') -> None:  # noqa
        ...
        engine.last_checkpoint_path = path
        optimizer, last_iter = None, None
        if engine._whole_cfg.learner.get('load_optimizer', True):
            optimizer=engine.optimizer
        if engine._whole_cfg.learner.get('load_last_iter', True):
            last_iter=engine.last_iter

        if path == '':  # not load
            return
        if not os.path.exists(path):
            print(f"Can't load from path:{path}")
            return
        ...
        engine.lr_scheduler.last_epoch = engine.last_iter.val
...
class SaveCkptHook(LearnerHook):
    ...
    def __call__(self, engine: 'BaseLearner') -> None:  # noqa
        ...
        if engine.rank == 0 and engine.last_iter.val % self._freq == 0:
            if engine._whole_cfg.common.type == 'rl':
                dirname = os.path.join(os.getcwd(), 'experiments', engine.experiment_name, engine._whole_cfg.learner.player_id, 'checkpoint')
                if engine._whole_cfg.learner.get('save_to_lustre',False):
                    dirname = dirname.replace('/mnt/cache','/mnt/lustre')
            else:
                dirname = os.path.join(os.getcwd(), 'experiments', engine.experiment_name, 'checkpoint')
            if not os.path.exists(dirname):
                try:
                    ...
                    pass
            if engine._whole_cfg.common.type == 'rl':
                path = os.path.join(dirname, '{}_{}_iteration_{}.pth.tar'.format(engine.experiment_name, engine._whole_cfg.learner.player_id, engine.last_iter.val))
            else:
                path = os.path.join(dirname,
                                    '{}_iteration_{}.pth.tar'.format(engine.experiment_name, engine.last_iter.val))
            ...
            engine.last_checkpoint_path = path
            engine.info('{} save checkpoint in {}'.format(engine.name, path))
...
class LogShowHook(LearnerHook):
    ...
    def __call__(self, engine: 'BaseLearner') -> None:  # noqa
        ...
        frames = int(iters * engine._world_size * engine._whole_cfg.learner.data.batch_size * engine._whole_cfg.learner.data.trajectory_length)
    ...
    def __call__(self, engine: 'BaseLearner') -> None:  # noqa
        ...
        def aggregate(data):
            ...
            if isinstance(data, dict):
                ...
                new_data = data.clone().detach().cuda()
                ...
                new_data = torch.scalar_tensor(data).reshape([1]).cuda()
                allreduce(new_data)
                new_data = new_data.cpu().item()
            else:
