...
See the [scripted agents](../ctools.pysc2/agents/scripted_agent.py) for an example of
...
The full list is defined in `ctools.pysc2.lib.features`.
...
*   **player_relative**: Which units are friendly vs hostile. Takes values in
    [0, 4], denoting [background, self, ally, neutral, enemy] units
    respectively.
*   **unit_type**: A unit type id, which can be looked up in distar.ctools.pysc2/lib/units.py.
*   **selected**: Which units are selected.
...
`ValidActions` in `ctools.pysc2.lib.actions`, and then each observation specifies which
of the available function is valid this frame. Each action is a single
`FunctionCall` in `ctools.pysc2.lib.actions` with all its arguments filled.

The full set of types and functions are defined in `ctools.pysc2.lib.actions`. The set
...
```shell
$ python -m distar.ctools.pysc2.bin.valid_actions
```
...
function call. The exact definitions are in `ctools.pysc2.lib.actions.TYPES`
...
In `ctools.pysc2.lib.actions.FUNCTIONS` specific functions have an additional parameter
that references the general parameter.

#### Example usage

Take a look at the [random agent](../ctools.pysc2/agents/random_agent.py) for an
...
from distar.pysc2 import actions
...
The main SC2 environment is at `ctools.pysc2.env.sc2_env`, with the action and
observation space defined in `ctools.pysc2.lib.features`.

The most important argument is `map_name`, which is how to find the map.
Find the names by using `ctools.pysc2.bin.map_list` or by looking in `ctools.pysc2/maps/*.py`.
